...
- Never call `createGitHubRemediationPr` when `isGitFlowDemoMode` is true: `DEMO_MODE`, missing `GITHUB_TOKEN`, or auditor not `LIVE`. Return `@cloudpulse/gitflow/mocks` (`MOCK_DRAFT_PR`) instead.
...
- Octokit only on the live path. Do not rename `DEMO_MODE` / `USE_LIVE_AWS` / `SIMULATED`|`LIVE`.
- Tests: Vitest.
