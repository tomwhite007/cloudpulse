...
- Composition roots (Nest `useFactory`, Next `route.ts`) MAY import mocks and return them. They MUST NOT define mock datasets, factories, path switches, or response builders in the live file. A demo/unauthenticated branch is a thin call only: `return NextResponse.json(MOCK_X)` or `return createMockY(...)`. Request parsing and per-path mock payloads belong in `mocks/<domain>.mock.ts`.
