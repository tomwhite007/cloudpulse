...
} from "../app/cloud-auditor.interface";
  ...
  constructor(
    ...
    this.server.tool(
      ...
      async ({ resourceId, resourceType, actionType, targetBranch }) => {
        ...
        const hclDiff =
          resource.recommendedAction?.terraformPatchPreview ||
          "/* No patch preview available */";
        const estimatedMonthlySavingsUsd =
          resource.potentialMonthlySavings || 0;
        const safetyChecks = [
          "Pre-flight snapshot confirmed",
          "No active IOPS for 30 days",
        ];
