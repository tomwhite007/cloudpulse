...
describe('parseEnv', () => {
  it('defaults the auditor API URL when the public var is missing', () => {
    expect(parseEnv({}).NEXT_PUBLIC_AUDITOR_API_URL).toBe(
      'http://localhost:3000',
    );
  });
  ...
  it('treats an empty public var as missing', () => {
    expect(
      parseEnv({ NEXT_PUBLIC_AUDITOR_API_URL: '' }).NEXT_PUBLIC_AUDITOR_API_URL,
  ...
  it('rejects a non-URL public var', () => {
    expect(() =>
      parseEnv({ NEXT_PUBLIC_AUDITOR_API_URL: 'not-a-url' }),
    ).toThrow();
  });
});
